...
class DataManager(object):
    ...
    def load_configuration(self) -> dict:
        ...
        try:
            with open(opj(self.file_path, "Libsystem_Config.json"), "r", encoding='utf-8') as f:
                config = json.load(f)
            ...
            # config 파일 저장
            with open(opj(dir_path, "Libsystem_Config.json"), "w", encoding='utf-8') as f:
    ...
    def check_author_validate(self, parts_used,value):
        ...
        if error_messages and parts_used == "수정":
            error_messages += "ERROR: 입력한 저자 중 식별번호가 없는 항목이 포함되어 있습니다.\n"
    ...
    def update_book(self):
        ...
        print("책의 수정될 저자를 입력해주세요. (여러 명일 경우 &로 구분)")
        new_author = input(input_message).strip()
        if new_author == self.config["cancel"]:
            print("수정을 중단하며 메인 프롬프트로 돌아갑니다.")
            return False
        new_author = self.check_author_validate("수정", new_author) # 저자 유효성 검사
        if not new_author:
            return False
